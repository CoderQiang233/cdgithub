...
class TPoliticalCensorship extends React.Component {
  constructor(props) {
  ...
  onSubmit = (data) => {
    ...
    console.log(data)
    this.props.dispatch(
      { type: 'matterTPolitical/uploadTable', payload: data }
    )
  }
 // 判断是否登录
 isLogin = (key) => {
                ...
                <table className={styles.serviceTable}>
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>教职工登录山西财经大学网上办事大厅→选择教职工政治审查证明（无犯罪证明）手续办理事项→填写《山西财经大学教职工政治审查表》→提交所在单位（部门）负责人（学院党委书记）填写申请人的现实政治表现并签字→提交保卫处综合科填写政审意见并签字→本人携带身份证原件在阳光服务大厅保卫处窗口办理。（委托办理需要提供委托书及双方身份证原件）</td>
                    </tr>
                    <tr>
          ...
          <table className={styles.table}>
            <tbody>
              ...
              <tr>
                ...
                <td className={styles.tdTitle}>单位（部门）</td>
                <td>
                  <FormItem>
                    {getFieldDecorator('unit', {
                      rules: [{ required: true, message: '请输入您的单位（部门）' }],
                    })(
                      <Input placeholder="请输入您的单位（部门）" />
                      )}
                  </FormItem>
                </td>
              </tr>
              ...
              <tr> 
                <td className={styles.tdTitle}>户籍所在地</td>
                <td colSpan='5'>
                  <FormItem>
                    {getFieldDecorator('native_place', {
                      rules: [{ required: true, message: '请输入您的户籍所在地' }],
                    })(
                ...
                <td colSpan='5'>
                  <FormItem>
                    {getFieldDecorator('political_performance', {
                      ...
                      <TextArea placeholder="内容应包括教职工的政治态度，特别是对党的十一届三中全会以来的路线、方针和政策的态度；在重大政治斗争中的表现，特别是在“文化大革命”和1989年政治风波中的表现以及近年来同“法轮功”邪教斗争中的表现；有无犯罪记录等方面。" rows={5} />
...
;
export default connect(mapStateToProps)(TPoliticalCensorship);
